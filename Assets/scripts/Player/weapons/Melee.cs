...
public class Melee : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other) {
        if(other.CompareTag("Enemy")) {
            ...
            if(!enemy.dead) {
                enemy.TakeDamage(weaponDefinition.damage, weaponDefinition.stealthWeapon, transform.root);
                objectPooler.SpawnFromPool(hitParticlesTag, other.ClosestPoint(transform.position), transform.rotation);
    ...
    {
        foreach (Transform ability in combat.currentAbilities)
        {
            if(ability.GetComponent<ShieldAbility>())
            {
                return ability.GetComponent<ShieldAbility>().blocking;
            }
        }
